...
| `clap` | `4.6.4` | crates.io; MIT OR Apache-2.0 | `pi-cli`: exact milestone-1 argument grammar, help text, and typed parse errors | Defaults off; `std`, `help`, `usage`, `error-context`, and `suggestions`; no derive/build script/native ABI |
...
| `tokio` | `1.53.1` | crates.io; MIT | `pi-agent`, `pi-provider`, `pi-store`, `pi-test-support`, `pi-tools`, `pi-cli`: bounded event delivery, async network/files/processes, credential-command output/time limits, synchronization, cancellation, and Unix signal handling | `macros`, `rt-multi-thread`, `sync`, `time`, `net`, `io-util`, `fs`, `process`, `signal`; proc-macro plus platform socket/process dependencies; no Tokio native ABI |
...
| `time` | `0.3.54` | crates.io; MIT OR Apache-2.0 | `pi-cli`: UTC RFC3339 session and entry timestamps | Defaults off; `std` and `formatting`; no native ABI; proc-macro features are disabled |
...
| `uuid` | `1.24.0` | crates.io; MIT OR Apache-2.0 | `pi-cli`: UUIDv7 session IDs and collision-resistant v4 entry IDs | `v4` and `v7`; randomness comes from the platform `getrandom` backend; no native ABI |
