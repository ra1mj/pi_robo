...
| `serde_json` | `1.0.151` | crates.io; MIT OR Apache-2.0 | `pi-agent`, `pi-protocol`, `pi-runtime` tests, `pi-test-support`, `pi-tools`: JSON values, fixtures, preservation tests, schemas, and tool details | Default; metadata/config build script; upstream contains unsafe internals but no native ABI |
...
| `tokio` | `1.53.1` | crates.io; MIT | `pi-agent`, `pi-provider`, `pi-test-support`, `pi-tools`: bounded event delivery, async network/files/processes, timeouts, synchronization, and cancellation signaling | `macros`, `rt-multi-thread`, `sync`, `time`, `net`, `io-util`, `fs`, `process`; proc-macro plus platform socket/process dependencies; no Tokio native ABI |
